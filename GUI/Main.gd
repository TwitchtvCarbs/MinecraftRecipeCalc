...
func _on_LineEdit_text_entered(new_text):
	var recentCount = $VBox/OptionButton.get_item_count()
	...
	if Global_Recipe_Create_Database.recipe_output != "This recipe does not exist!":
			...
			#for i in recentCount:
				#if $VBox/OptionButton.get_item_text(i) != new_text or $VBox/OptionButton.get_item_text(0) !=tempText:
					#$VBox/OptionButton.add_item(tempText.to_lower())
		#else:
			#if $VBox/OptionButton.get_item_text(0) != new_text or $VBox/OptionButton.get_item_text(0) !=tempText:
				$VBox/OptionButton.add_item(tempText.to_lower())
